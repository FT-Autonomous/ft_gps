...
class GPSNode(Node):
    def __init__(self):
        ...
        self.path_plot, = self.ax.plot([], [], 'ko-', label='GPS Path')
        self.ax.set_title("Live GPS Plot")
    ...
    def draw_track(self):
        self.track_fig, self.track_ax = plot.subplots(figsize=(10, 10))

        def plot_cones(cones, color, label):
            xs, ys = zip(*cones) if cones else ([], [])
            self.track_ax.scatter(xs, ys, c=color, label=label, s=50)
        ...
        self.track_ax.set_title("Track Layout")
        self.track_ax.set_xlabel("Local X (meters)")
        self.track_ax.set_ylabel("Local Y (meters)")
        self.track_ax.set_xlim(____, ____)  # ← fill in desired X limits
        self.track_ax.set_ylim(____, ____)  # ← fill in desired Y limits
        self.track_ax.grid(True)
        self.track_ax.legend()

        self.track_fig.canvas.draw()
        self.track_fig.canvas.flush_events()
            ...
            if line.startswith("$GNGGA"):
                ...
                if len(parts) > 5 and parts[6] != '0':
                    ...
                    if self.origin_lat is None:
                        ...
                        # Update live plot
                        self.path_plot.set_data(self.gps_x, self.gps_y)
                        self.ax.set_xlim(-30, 30)  # ← set your desired x-limits
                        self.ax.set_ylim(-35, 35)  # ← set your desired y-limits
